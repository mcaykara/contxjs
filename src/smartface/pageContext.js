...
commands.addRuntimeCommandFactory(function pageContextRuntimeCommandFactory(type, error) {
	...
	const isTablet = require('../smartface/isTablet');
	...
	switch (hook) {
		case 'beforeStyleDiffAssign':
			return function beforeStyleDiffAssign(styles) {
				// for testing
				return require("./sfCorePropFactory")(styles);
			};
 ...
 * 
 * @return {function} - Styles update wrapper
 */
function createPageContext(component, name, reducers = null) {
	...
	/**
	 * Creates new StyleContext with given styler
	 * 
	 * @params {function} styling - Styler styling wrapper
	 */
...
export function pageContextReducer(context, action, target, state) {
    ...
    case 'pushClassNames':
    	if(!action.classNames)
    		throw new TypeError("Classnames must not be null or undefined");
